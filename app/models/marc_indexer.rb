...
class MarcIndexer < Blacklight::Marc::Indexer
  ...
  def initialize
    ...
    to_field 'isbn_tesim', extract_marc('020a', separator: nil) do |_rec, acc|
      orig = acc.dup
      acc.map! { |x| StdNum::ISBN.allNormalizedValues(x) }
      acc << orig
      acc.flatten!
    ...
    to_field 'subject_topic_facet', extract_marc('600abcdq:610ab:611ab:630aa:650aa:653aa:654ab:655ab',
    ...
    to_field 'location_tesim' do |rec, acc|
      rec.each_by_tag('926') do |field|
        acc << locations[field['m'].downcase.delete('_').delete('-')]
      end
    end
